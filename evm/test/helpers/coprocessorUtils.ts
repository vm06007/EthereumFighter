...
// Extend HardhatRuntimeEnvironment to include __SOLIDITY_COVERAGE_RUNNING
...
export function insertSQL(handle: string, clearText: bigint, replace: boolean = false): void {
  if (replace) {
    ...
    function executeQuery() {
      db.get("SELECT clearText FROM ciphertexts WHERE handle = ?", [handleStr], (err, row) => {
        if (err) {
          ...
          resolve((row as { clearText: string }).clearText);
...
interface FHEVMEvent {
  eventName: string;
  args: { [key: number]: any }; // Explicitly define args as an indexed object
}
...
async function processAllPastTFHEExecutorEvents(): Promise<void> {
  ...
  const events = logs
    ...
    .filter((event) => event !== null) as FHEVMEvent[];
...
async function insertHandleFromEvent(event: FHEVMEvent): Promise<void> {
  let handle: string;
  ...
  switch (event.eventName) {
    ...
    case "FheAdd":
      ...
      clearLHS = BigInt(await getClearText(event.args[0]));
      if (event.args?.[2] === "0x01") {
        clearText = BigInt(clearLHS) + BigInt(event.args[1]);
        clearText = clearText % 2n ** NumBits[resultType as keyof typeof NumBits];
      } else {
        clearRHS = BigInt(await getClearText(event.args[1]));
    ...
    case "FheShl":
      ...
      clearLHS = BigInt(await getClearText(event.args[0]));
      if (event.args[2] === "0x01") {
        clearText = BigInt(clearLHS) << BigInt(event.args[1]) % NumBits[resultType as keyof typeof NumBits];
        clearText = clearText % 2n ** NumBits[resultType as keyof typeof NumBits];
      } else {
        clearRHS = BigInt(await getClearText(event.args[1]));
    ...
    case "FheRotl":
      ...
      clearLHS = BigInt(await getClearText(event.args[0]));
      if (event.args[2] === "0x01") {
        shift = BigInt(event.args[1]) % NumBits[resultType as keyof typeof NumBits];
        clearText = (BigInt(clearLHS) << shift) | (BigInt(clearLHS) >> (NumBits[resultType as keyof typeof NumBits] - shift));
        clearText = clearText % 2n ** NumBits[resultType as keyof typeof NumBits];
      } else {
        clearRHS = BigInt(await getClearText(event.args[1]));
    ...
    case "FheMax":
      ...
      clearLHS = BigInt(await getClearText(event.args[0]));
      if (event.args[2] === "0x01") {
        clearText = BigInt(clearLHS) > BigInt(event.args[1]) ? clearLHS : BigInt(event.args[1]);
      } else {
        clearRHS = BigInt(await getClearText(event.args[1]));
    ...
    case "Cast":
      ...
      handle = ethers.toBeHex(event.args[2], 32);
      clearText = BigInt(await getClearText(event.args[0])) % 2n ** NumBits[resultType as keyof typeof NumBits];
      insertSQL(handle, clearText);
      break;
...
export function getFHEGasFromTxReceipt(receipt: ethers.TransactionReceipt): number {
  ...
  const contract = new ethers.Contract(executorAddress, abi, hre.ethers.provider);
  ...
  for (const event of FHELogs) {
    ...
    switch (event.name) {
      ...
      case "FheLe":
        ...
        type = parseInt(handle.slice(-4, -2), 16);
        if (event.args?.[2] === "0x01") {
          const test = operatorPrices["fheLe"].scalar[type]
          FHEGasConsumed += 10;
        } else {
